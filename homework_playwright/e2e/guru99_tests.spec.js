...
test('test Categories surfing', async ({ page }) => {
    ...
    await expect(page.locator('//*[@id="menu-item-4744"]/a')).toBeVisible({ timeout: 10000 });
...
test('test Localization', async ({ page }) => {
  ...
  test('test HomeTask in Headless Mode', async ({ page }) => {
      ...
      await page.screenshot({ path: 'headless-test-screenshot.png' });
    //   await page.goto('https://www.guru99.com/').waitForSelector('.gsc-input', { state: 'visible'})

    //   const searchInput2 = await page.waitForSelector('.gsc-input-box', { state: 'visible', timeout: 30000 });
